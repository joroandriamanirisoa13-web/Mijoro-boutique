...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpvZ29oa2Z6cGxjdW9ua2tmb292Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjA4Nzk0ODAsImV4cCI6MjA3NjQ1NTQ4MH0.AeQ5pbrwjCAOsh8xxxxxxxxxxxxxxxxxxx';
...
loginForm?.addEventListener('submit', async (e)=>{
  ...
  else { loginModal?.close(); await initAuth(); }
});
...
async function saveProduct(p){
  ...
  const payload = {
    title: p.title, type: p.type, is_free: p.is_free, price: p.price, promo: p.promo, is_vip: p.is_vip,
...
async function render(){
  ...
  for(const p of items){
    ...
    if(isOwner){
      ...
      const del = document.createElement('button'); del.className='btn'; del.textContent='Delete'; del.onclick=async()=>{if(confirm(`Supprimer "${p.title}" ?`)){await supabase.from('products').delete().eq('id',p.id); await render();}};
